...
def register_user():
    ...
    try:
        ...
        if new_user.user_type == UserType.CUSTOMER:
            profile = CustomerProfile(user_id=new_user.id, first_name=data['first_name'], last_name=data['last_name'])
        elif new_user.user_type == UserType.PROVIDER:
            profile = ProviderProfile(user_id=new_user.id, company_name=data['company_name'])
        else:
            db.session.rollback()
    ...
    return jsonify({"message": "User and profile registered successfully!", "user": {"id": new_user.id, "email": new_user.email, "user_type": new_user.user_type.value}}), 201
...
def get_profile():
    ...
    if user.user_type == UserType.CUSTOMER:
        ...
        return jsonify({"id": user.id, "email": user.email, "first_name": profile_data.first_name, "last_name": profile_data.last_name})
    elif user.user_type == UserType.PROVIDER:
        profile_data = user.provider_profile
        return jsonify({"id": user.id, "email": user.email, "company_name": profile_data.company_name, "is_verified": profile_data.is_verified})
...
def add_venue():
    ...
    return jsonify({"message": "Venue created successfully and is under review.", "venue_id": new_venue.id}), 201

# app.py (replace the existing functions with these)

@app.route('/venues', methods=['GET'])
@jwt_required()  # <-- 1. Add decorator to protect the route
def get_all_venues():
    """Protected endpoint for customers to get a list of all venues."""
    # --- 2. Add Role Check ---
    current_user_id = get_jwt_identity()
    user = User.query.get(current_user_id)
    if not user or user.user_type != UserType.CUSTOMER:
        return jsonify({"msg": "Access forbidden: Customers only"}), 403
    # --- End of Role Check ---
...
def get_venue_details(venue_id):
    """Protected endpoint for customers to get details of a specific venue."""
    # --- 2. Add Role Check ---
    current_user_id = get_jwt_identity()
    user = User.query.get(current_user_id)
    if not user or user.user_type != UserType.CUSTOMER:
...
@app.route('/provider/venues', methods=['GET'])
@jwt_required()
def get_provider_venues():
    ...
    if user.user_type != UserType.PROVIDER:
        return jsonify({"msg": "Access forbidden: Providers only"}), 403
    provider_venues = Venue.query.filter_by(provider_id=user.id).all()
    venue_list = [venue.to_dict() for venue in provider_venues]
    return jsonify(venue_list), 200
...
def update_venue(venue_id):
    ...
    if str(venue.provider_id) != current_user_id:
        return jsonify({"msg": "Forbidden: You do not own this venue"}), 403
...
def seed_data():
    ...
    if not VenueCategory.query.first():
        print("Creating venue categories...")
        categories = [
            VenueCategory(name='Wedding Hall', description='Spacious halls for weddings.'),
            VenueCategory(name='Conference Room', description='Professional spaces for meetings.'),
            VenueCategory(name='Outdoor Space', description='Open areas for various events.')
        ]
